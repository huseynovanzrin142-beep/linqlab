...
public class DebtorRegistry
{
    #region Debtors's List 
    public List<Debtor> Debtors { get; private set; } = new List<Debtor>()
        {
    ...
    #endregion
    public void AddNewDebtor(Debtor anyDebtor)
    {
        foreach (var debtor in Debtors)
        {
            if (debtor.Email.Equals(anyDebtor.Email)) throw new DebtorAlreadyExitException($"A debtor with the email '{anyDebtor.Email}' already exists in the registry.");
        }
        Debtors.Add(anyDebtor);
    }
